...
- 투입 직원: plannings / develops / arts (EmployeeSO[]) (임시로 EmployeeSO 사용)
- 진행도: progress (float) → 기본 0(시작) ~ 240(목표) → 0~100% 으로 표시
  - 완성도 점수: qualityScore (float) → 기획
  - 안정성 점수: stabilityScore (float) → 개발
  - 매력도 점수: charmScore (float) → 아트
  - 이벤트 가중치 누적: 미구현
- isFinished (bool)
